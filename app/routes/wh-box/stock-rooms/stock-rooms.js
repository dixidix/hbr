function stockRoomsController(angular, app) {
    ...
    stockRoomsCtrl.$inject = ['$scope','$rootScope','$filter', '$http', 'boxService', '$uibModal', '$state', 'authenticationService'];
    ...
    function stockRoomsCtrl($scope,$rootScope,$filter, $http, boxService, $uibModal, $state, authenticationService) {
        ...
        function getAllBoxes(isAdmin) {
            authenticationService.checkAuth().then(function(response) {
                ...
                boxService._getAllEnterBoxes().then(function(response) {
                        ...
                        if ($state.params.spBoxId) {
                            setFilter('shipping_box_id', self.lang.room.filter.list.shipping_box_id);
                            self.searchBy = $state.params.spBoxId;
                            applyFilter();
                        }
                    })
                    .catch(function(err) {
        ...
        function init() {
            ...
            self.lang = $rootScope.langs.stock_rooms;            
