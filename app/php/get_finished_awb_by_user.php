...
if(!empty($_GET['state'])){
	$res = MysqliDB::getInstance()->query("SELECT * from airway_bill INNER JOIN ventas ON airway_bill.ventaId = ventas.id  WHERE airway_bill.state != 0 AND airway_bill.state != 1 AND ventas.uid = ".$_GET['uid']." AND  airway_bill.deleted = 0");
} else {
