...
if($action === "whLeaveDate"){
    ...
    $body = file_get_contents('./../emails/wh-leave-date-user.template.html', FILE_USE_INCLUDE_PATH);    
    ...
    $htmlStringToReplace = array('$ventaId','$guide_number', '$whLeaveDateParsed', '$wh_name');
    $replaceWith   = array("$ventaId","$guide_number", "$whLeaveDateParsed", "$wh_name");

    $body = str_replace($htmlStringToReplace, $replaceWith, $body);

    $to = "santiago.lloret@tucourier.com.ar";
    $name = "HBR | tu courier";
    $subject = "Fecha de salida de warehouse para la guía: ".$guide_number." perteneciente al lote de compra N°: ".$ventaId;
    $mail->CharSet = 'UTF-8';
    ...
    $mail->Body    = $body;
    $mail->AltBody = "Fecha de salida de warehouse para la guía: ".$guide_number." perteneciente al lote de compra N°: ".$ventaId;
    if(!$mail->send()) {
...
if($action === "whEnterDate"){
    ...
    $body = file_get_contents('./../emails/wh-arrival-date-user.template.html', FILE_USE_INCLUDE_PATH);    
    ...
    $htmlStringToReplace = array('$ventaId','$bill_number', '$whEnterDateParsed','$whDateNotificationText', '$wh_name');
    $replaceWith   = array("$ventaId","$bill_number", "$whEnterDateParsed","$whDateNotificationText", "$wh_name");

    $body = str_replace($htmlStringToReplace, $replaceWith, $body);

    $to = "santiago.lloret@tucourier.com.ar";
    $name = "HBR | tu courier";
    $subject = "Warehouse Arrival Date Lot: ".$ventaId." - Bill N°: ".$bill_number;
    $mail->CharSet = 'UTF-8';
    $mail->AddReplyTo($email);
    $mail->SetFrom($to, $name);
    $mail->Subject = $subject;
    $mail->AddAddress($email);
    $mail->Body    = $body;
    $mail->AltBody = "Warehouse Arrival Date Lot: ".$ventaId." - Bill N°: ".$bill_number;
