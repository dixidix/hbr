function shippingController(angular, app) {
    ...
    shippingCtrl.$inject = ['shippingService', 'boxService', '$scope', '$rootScope', '$uibModal', '$state','$filter', '$http', '$sce', '$compile'];
    ...
    function shippingCtrl(shippingService, boxService, $scope, $rootScope, $uibModal, $state,$filter, $http, $sce, $compile) {
        ...
        function deleteBox(box, index) {
            confirmModal('./hbr-selfie/dist/routes/wh-box/shipping/confirm/delete.confirm.html');
            ...
            $scope.deleteAccept = function() {
                shippingService._deleteBox(box).then(function(res){
                    $state.go('dashboard.shipping', {}, { reload: true });
        ...
        function saveLocation(box) {
            ...
            confirmModal('./hbr-selfie/dist/routes/wh-box/shipping/confirm/location.confirm.html');
            ...
            $scope.locationAccept = function() {
                ...
                box.wh_arrival_date = $scope.spbox.wh_arrival_date || box.wh_arrival_date || null;
                box.wh_leave_date = $scope.spbox.wh_leave_date || box.wh_leave_date || null;
                box.wh_provider = $scope.spbox.wh_provider || box.wh_provider || box.provider || null;
                box.wh_tracking = $scope.spbox.wh_tracking || box.wh_tracking || box.tracking || null;
                box.travel_status = 1;
        ...
        function dragEnter(status, box) {
            ...
            if (box.status != status && box.id == self.prevBox) {
                confirmModal('./hbr-selfie/dist/routes/wh-box/shipping/confirm/status.confirm.html');
                ...
                $scope.statusAccept = function() {
                    ...
                    if(status == 0 || status == 1){
                        box.wh_tracking = $scope.confirmBox.tracking || box.wh_tracking || null;
                        box.tracking = $scope.confirmBox.tracking || box.tracking || null;
                        box.wh_provider = $scope.confirmBox.provider || box.wh_provider || null;
                        box.provider = $scope.confirmBox.provider ||  box.provider || null;

                        if(status == 0) {
                            box.wh_arrival_date = $scope.confirmBox.arrival_date || box.wh_arrival_date || null;
                            box.arrival_date = $scope.confirmBox.arrival_date || box.arrival_date || null;
                            box.wh_leave_date = $scope.confirmBox.leave_date || box.wh_leave_date || null;
                            box.leave_date = $scope.confirmBox.leave_date ||  box.leave_date || null;
                        }
    
                        if(status == 1) {
                            box.bsas_arrival_date = $scope.confirmBox.arrival_date || box.bsas_arrival_date || null;
                            box.arrival_date = $scope.confirmBox.arrival_date || box.arrival_date || null;
                            box.wh_leave_date = $scope.confirmBox.leave_date || box.wh_leave_date || null;
                            box.leave_date = $scope.confirmBox.leave_date || box.leave_date || null;
                        }
                    ...
                    if(status == 2){
                        box.bsas_tracking = $scope.confirmBox.tracking || box.bsas_tracking || null;
                        box.tracking = $scope.confirmBox.tracking || box.tracking || null;
                        box.bsas_provider = $scope.confirmBox.provider || box.bsas_provider || null;
                        box.provider = $scope.confirmBox.provider || box.provider || null;

                        box.customer_arrival_date = $scope.confirmBox.arrival_date || box.customer_arrival_date || null;
                        box.arrival_date = $scope.confirmBox.arrival_date || box.arrival_date || null;
                        box.bsas_leave_date = $scope.confirmBox.leave_date || box.bsas_leave_date || null;
                        box.leave_date = $scope.confirmBox.leave_date || box.leave_date || null;
                    }
        ...
        function navigate(box) {
            $scope.hovered = '';
            $state.go('dashboard.stock_rooms', { spBoxId: box.id }, { reload: true });
        }
        ...
        function calculateValue(spBox) {
            ...
            var total = {
                ...
                quantity: 0,
                wh_val: 0.00,
                wh_aditional_val: 0.00
            }

            spBox.enter_box.forEach(function(box) {
                total.weight = parseFloat(total.weight) + parseFloat(box.box_partial_weight);
                total.value = parseFloat(total.value) + parseFloat(box.box_partial_value);
                total.quantity = parseInt(total.quantity) + parseInt(box.quantity);
                total.wh_val = parseFloat(total.wh_val) + parseFloat(box.box_warehouse_value);
                total.wh_aditional_val = parseFloat(total.wh_aditional_val) + parseFloat(box.aditional_total);
            });
