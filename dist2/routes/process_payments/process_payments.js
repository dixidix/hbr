function processPaymentsController(angular, app) {
                        ...
                        .then(function(response) {
                            if (response.data.ventas) {
                                angular.forEach(response.data.ventas, function(value, key) {
                                    ...
                                    response.data.ventas[key].dtOrder = value.timestamp;
        ...
        function generateToken() {
            $http.post('./hbr-selfie/dist/php/encrypt.php', { timestamp: Math.round((new Date()).getTime() / 1000) })
                .then(function success(response) {
                    self.successUrl = "https://tucourier.com.ar/hbr-selfie/dashboard/shopping/checkout/success/" + response.data;
    ...
    function makeGuidesModalCtrl($scope, $state, $filter, $uibModalInstance, $uibModal, $sce, $compile, $rootScope, venta, bills, $http, airwayService, $q) {
        ...
        function updateWhDate(whDate, index) {
            var bill = angular.copy(self.venta.bills[index]);
            bill.wh_enter_date = new Date(whDate).getTime() / 1000 || null;
            bill.set_wh_date = true;
            self.venta.bills[index].spinner = true;
            return airwayService.updateBill(bill).then(function(response) {
                $http.post('./hbr-selfie/dist/php/notifications/wh_notifications.php', {
                    ventaId: self.venta.id,
                    bill_number: bill.number,
                    wh_enter_date: bill.wh_enter_date,
                    email: bill.warehouse[0].email,
                    name: bill.warehouse[0].name,
                    user: self.venta.user[0].name + " " + self.venta.user[0].lastname,
                    action: 'whEnterDate',
                }).then(function success(response) {
                    $http.post('./hbr-selfie/dist/php/notifications/wh_notifications_user.php', {
                        ...
                        wh_enter_date: bill.wh_enter_date,
                        name: bill.warehouse[0].name,
                        email: self.venta.user[0].email,
                        action: 'whEnterDate',
                    }).then(function() {
                        self.venta.bills[index].spinner = false;
                        self.venta.bills[index].changed = false;
                        self.venta.bills[index].saved = true;
                    })
                });
            });
        }
        ...
        function save() {
            ...
            angular.forEach(self.guideBatch, function(awb) {
                if (awb.products.length > 0) {
                    sequence = sequence.then(function() {
                        return airwayService.save(awb).then(function(response) {
                            angular.forEach(awb.products, function(product) {
                                product.airwayId = response.data.airwayId;
        ...
        function updateVenta() {
            ...
            angular.forEach(self.venta.bills, function(bill) {
                ...
                sequence = sequence.then(function() {
                    bill.wh_enter_date = new Date(bill.wh_enter_date).getTime() / 1000 || null;
                    return airwayService.updateBill(bill).then(function(response) {
                        angular.forEach(bill.products, function(product) {
                            airwayService.updateProduct(product);
                        });
            ...
            self.venta.guide_amount = self.guideBatch.length;
            airwayService.updateVenta(self.venta).then(function(response) {
                $uibModalInstance.dismiss('cancel');
                $state.go('dashboard.process_payments', { reload: true });
            });
        }
        ...
        function init() {
            $http.post('./hbr-selfie/dist/php/update_batch_status.php', { id: venta.id, readed: '1' });
            self.updateWhDate = updateWhDate;
            self.cancel = cancel;
            self.venta = venta;
            self.guideNumber = 0;
            self.bills = bills;
            ...
            angular.forEach(self.bills, function(bill) {
                bill.isOpen = false;
                bill.wh_enter_date = new Date(parseInt(bill.wh_enter_date) * 1000);
            });
            self.deleted = false;
